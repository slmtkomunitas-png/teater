...
import { warna, radius, inisial, warnaAvatar } from './theme';
...
export function Avatar({ nama, size = 44 }) {
  ...
  return (
    ...
    >
      <Text style={{ color: warna.white, fontWeight: '800', fontSize: size * 0.36 }}>
        {inisial(nama)}
      </Text>
    </View>
  );
...
export function Kartu({ children, style }) {
  return <View style={[styles.kartu, style]}>{children}</View>;
}

const styles = StyleSheet.create({
  avatar: { alignItems: 'center', justifyContent: 'center' },
  headerWrap: { backgroundColor: warna.bg, borderBottomWidth: 1, borderBottomColor: warna.border },
  chip: {
  ...
  headerTitle: { color: warna.teks, fontSize: 18, fontWeight: '800', letterSpacing: 0.1 },
  headerSubtitle: { color: warna.muted, fontSize: 13, marginTop: 2 },
  keluar: {
    borderWidth: 1, borderColor: warna.border, borderRadius: 99,
    paddingHorizontal: 14, paddingVertical: 7,
  },
  keluarText: { color: warna.muted, fontSize: 13, fontWeight: '700' },
  kartu: {
